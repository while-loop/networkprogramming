...
class Decoder {
	...
	/**
	 * The buffer with encoded java.data
	 */
	byte[] data;
	/**
	 * The offset in java.data where the current decoder head is positioned
	 */
	int offset;
	/**
	 * The length of the current component being decoded, after which null is returned even if "java.data" has more java.data
	 */
	int length;
	...
	public Decoder(byte[] data){
		if(data==null) throw new RuntimeException("parameter java.data should not be null");
	...
	public int contentLength() {
		...
		if (ASN1_Util.getUnsignedShort(data[offset+tlen]) < 128) {
			//System.err.println("in ContentLength: len_len=="+java.data[offset+tlen]+" off="+offset);
			return data[offset+tlen];
		}
		//System.err.println("Len_len="+java.data[offset+tlen]+" gs="+gs(java.data[offset+tlen]));
	...
	public int lenLen(){
		//System.err.println("lenLen:"+ASN1_Util.byteToHex(java.data, offset, 5, " "));
		int tlen = typeLen();
		//System.err.println(tlen+"=tlen +length byte="+java.data[offset+tlen]);
	...
	public Decoder getFirstObject(boolean extract) {
		...
		if (new_len > length || new_len < 0) throw new ASNLenRuntimeException("ASN1:Decoding:Invalid object length ["+new_len+"]: Too long given available java.data:"+length);
		...
		//System.out.println("Decoder: extracts lens="+new_len+"<="+length +" off="+old_offset+"/"+java.data.length);
	...
	public boolean fetchAll(InputStream is) throws IOException {
		...
		while(true) {
			...
			if ((asrlen<0) || (length < asrlen)) {
				...
				length += inc;
				//dec = new Decoder(java.data,0,length);
				continue;
			}
			break;
		}
